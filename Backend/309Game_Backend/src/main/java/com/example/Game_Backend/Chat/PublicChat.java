 ...
 * <p>
 * Example URL: ws://coms-309-021.class.las.iastate.edu:8080/chat/username
 * <p>
...
public class PublicChat {
    ...
    // server side logger
    private final Logger logger=LoggerFactory.getLogger(PublicChat.class);
    ...
    public void onMessage(Session session,String rawMessage) throws IOException {
        ...
        } else if(message.startsWith("/friend")) {//ability to add friend in chat

            // split by space
            String[] split_msg = message.split("\\s");

            if(split_msg.length!=2) {//checks formatting
                sendMessageToPArticularUser(username,"[Error, bad formatting]: \"/friend username\" expected");
            }else {
                String friendName = split_msg[1];

                if(!usernameSessionMap.containsKey(friendName)){//makes sure friend exists
                    sendMessageToPArticularUser(username,"[Error, desired friend not found]: check spelling");
                }

                Long friendID=accountRepository.findByUsername(friendName).getId();
                Long userID=accountRepository.findByUsername(username).getId();
                Player user=playerRepository.findById(userID).orElseThrow(RuntimeException::new);
