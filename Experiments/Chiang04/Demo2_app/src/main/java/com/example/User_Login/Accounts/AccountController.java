...
//---------------------------------------//
    ...
    //This is the one that we are currently using
    ...
    if(accountRepository.findByUsername(newAccount.getUsername()) != null){
        Message a = new Message("username is already in use, please try another one", "true"); // or any other success message
    ...
    String StoringMapObjects(@RequestBody Account newAccount){
        ...
        if(accountRepository.findByUsername(newAccount.getUsername()) != null){
            String a = "{\"message\":" + "\"" + "username is already in use, please try another one" + "\""+ "," + "\"status\":\"true\"}";
            return a;
        }
...
//    }
    ...
    //This si the one that we are currently using
    @PostMapping("/login1")
